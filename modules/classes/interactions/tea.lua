...
function tea:draw()
    ...
    self.teapotRef, changed = ImGui.InputTextWithHint('##teapot', '$/mod/#teapot', self.teapotRef, 150)
    ...
    ImGui.SetCursorPosX(self.maxNodeRefPropertyWidth)
    style.setNextItemWidth(300)
    self.teacupRef, changed = ImGui.InputTextWithHint('##teacup', '$/mod/#teacup', self.teacupRef, 150)
    if changed then self.project:save() end
    ImGui.SameLine()
